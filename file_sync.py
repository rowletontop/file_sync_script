...
if not destination_files:
    print(f"No files found in {destination_folder}. Cannot compare files. Matching files will get updated.")
    for file in source_files:
        file_name = os.path.basename(file)
        copy_destination = os.path.join(destination_folder, file_name)
        try:
    ...
    destination_files_set = set(os.path.basename(file) for file in destination_files)
    new_files = source_files_set - destination_files_set
    for file in new_files:
        file_path = next((file_path for file_path in source_files if os.path.basename(file_path) == file), None)
        copy_destination = os.path.join(destination_folder, file)
        try:
    ...
    existing_files = source_files_set & destination_files_set
    for file in existing_files:
        source_file_path = next((file_path for file_path in source_files if os.path.basename(file_path) == file), None)
        destination_file_path = next((file_path for file_path in destination_files if os.path.basename(file_path) == file), None)

        if get_file_hash(source_file_path) != get_file_hash(destination_file_path):
            try:
